...
- **Datenschutz-Architektur (präzise):**
  ...
  - **Erweiterung – Outlook-Ordnerüberwachung (siehe Kernfunktion 12):** zusätzliche externe Abhängigkeit neben Claude, über die Microsoft Graph API. Mail-Inhalte (Betreff + Text) des konfigurierten Outlook-Ordners fließen dauerhaft in die lokale Project-A-Datenbank auf dem Hetzner-Server. Das UI zeigt in den Settings transparent an, ob und welcher Ordner aktuell überwacht wird.
...
- **E-Mail-Anbindung (Erweiterung, siehe Kernfunktion 12):** Microsoft Graph API (OAuth2, delegated permission `Mail.Read`) für die optionale Outlook-Ordnerüberwachung – bewusst nicht IMAP, da Microsoft IMAP/Basic-Auth zunehmend abschaltet. Periodisches Polling über einen einfachen In-Process-Scheduler (z. B. `APScheduler`) im FastAPI-Prozess, keine externe Job-Queue (siehe "Bewusst NICHT für den MVP einführen").
...
- **Document:** id, project_id, typ (meeting | systemeinstellung | prozess | notiz | datei | bild | sonstiges | email),
  titel,
  ...
  **outlook_message_id** (optional, nur bei automatisch per Outlook-Ordnerüberwachung angelegten Documents, siehe Kernfunktion 12 – eindeutige Microsoft-Graph-Message-ID, dient dort als mail-spezifische Duplikatserkennung anstelle von `datei_hash`, da keine Datei vorliegt),
...
- **EmailWatchConfig** *(pro Projekt, optional – nicht jedes Projekt hat zwingend eine Outlook-Anbindung, siehe Kernfunktion 12)*: project_id, outlook_ordner_id, outlook_ordner_name (zur Anzeige in den Settings), aktiv (bool), polling_intervall_minuten, letzte_abfrage_am (optional), letzter_fehler (optional). Fest **eine** Ordner-zu-Projekt-Zuordnung im MVP, kein Mapping mehrerer Ordner. OAuth-Access-/Refresh-Token werden getrennt und verschlüsselt gespeichert (siehe Einstellungen, Kernfunktion 12) – nicht Teil dieser Tabelle, damit Token-Rotation nicht die fachliche Konfiguration mitversioniert.
...
11. **Einstellungen (Settings-Seite fürs gesamte Programm):**
    ...
    - **Outlook-Ordnerüberwachung (siehe Kernfunktion 12):** Konfiguration von `EmailWatchConfig` pro Projekt (Ordnerauswahl/-ID, Polling-Intervall, aktiv/inaktiv), einmaliger OAuth-Login-Flow zur Anbindung des Microsoft-Kontos, Anzeige des letzten Abfragezeitpunkts und ggf. des letzten Fehlers.
    - Settings-Seite ist Teil des Basic-Auth-geschützten Bereichs.

12. **Outlook-Ordnerüberwachung (Erweiterung):** Project-A überwacht optional einen festen Outlook-Ordner und legt neue eingehende Mails automatisch als Document im zugeordneten Projekt an.
    - **Anbindung:** über die **Microsoft Graph API** (nicht IMAP – Microsoft schaltet IMAP/Basic-Auth zunehmend ab, Graph ist der zukunftssichere Weg). Dafür ist eine **App-Registrierung in Azure AD** nötig (delegated permission `Mail.Read`), einmalig einzurichten; die App erhält OAuth2 Access-/Refresh-Token nach einem einmaligen Login-Flow.
    - **Zuordnung:** fest konfiguriert – **ein** Outlook-Ordner (per Ordner-ID) ist **einem** Project-A-Projekt zugeordnet. Kein Mapping mehrerer Ordner im MVP. Konfiguration erfolgt in den Settings (siehe Kernfunktion 11).
    - **Abfrage-Mechanismus:** **Polling**, kein Webhook/Push im MVP (einfacher, kein öffentlicher Callback-Endpoint nötig). Konfigurierbares Intervall (Default z. B. alle 10 Minuten), Abfrage nutzt Microsoft Graph Delta-Query oder eine Filterung nach Empfangszeitpunkt, um nur wirklich neue Mails seit der letzten Abfrage zu holen, nicht jedes Mal den kompletten Ordner.
    - **Zeitliche Begrenzung (wichtig):** Mails, deren Empfangszeitpunkt **älter als 1 Woche** ist, werden **nicht** abgeholt/als Document angelegt – weder beim allerersten Poll nach Einrichtung einer `EmailWatchConfig` (kein rückwirkendes Einlesen des gesamten Ordnerbestands) noch bei einer verzögerten/nachgeholten Abfrage (z. B. nach längerem Server-Ausfall). Der Cutoff wird gegen den tatsächlichen Empfangszeitpunkt der Mail geprüft, nicht gegen den Zeitpunkt der Polling-Abfrage selbst.
    - **Was aus einer Mail wird:** Nur **Betreff + Text der Mail** (kein automatisches Einlesen von Anhängen im MVP) wird als neues Document mit `typ = email` angelegt: `titel` = Betreff, `inhalt` = Mail-Text (Klartext, HTML-Mails werden zu Klartext konvertiert), `dokumentdatum` = Empfangszeitpunkt der Mail (nicht der Zeitpunkt der Verarbeitung durch Project-A, siehe `dokumentdatum`-vs.-`erstellt_am`-Unterscheidung im Datenmodell), `quelle/dateiname` = Absenderadresse + Betreff, als Hinweis.
    - **Duplikatserkennung (mail-spezifisch, kein Datei-Hash wie bei Uploads):** Die eindeutige Outlook-Message-ID wird pro erzeugtem Document gespeichert (`outlook_message_id`, siehe Datenmodell); vor dem Anlegen eines neuen Documents wird geprüft, ob diese Message-ID im Projekt bereits verarbeitet wurde, um Duplikate bei überlappenden Polling-Abfragen zu vermeiden.
    - **Verarbeitung wie jeder andere Text-Upload:** Das erzeugte Document durchläuft dieselbe Ingestion-Pipeline wie manuell angelegte Notizen (Chunking, Embedding, Status `pending` → `processing` → `ready`/`failed`, siehe Statuslogik in Kernfunktion 4) – **kein Review-Schritt** nötig, da es sich um reinen Text ohne KI-Analyse handelt (kein `review_required`), analog zu manuellen Texteinträgen.
    - **Neue Datenmodell-Entität `EmailWatchConfig`** (siehe Datenmodell oben), pro Projekt, optional – nicht jedes Projekt hat zwingend eine Outlook-Anbindung.
    - **OAuth-Token-Speicherung:** Access-/Refresh-Token werden **verschlüsselt** in SQLite gespeichert, nach demselben Prinzip wie der Claude-API-Key (eigenes Encryption-Secret, das außerhalb der DB bleibt; das Secret darf, muss aber nicht dasselbe sein wie für den Claude-Key – technische Entscheidung, kurz dokumentieren). Token-Refresh läuft automatisch im Hintergrund vor Ablauf.
    - **Fehlerbehandlung:** Schlägt eine Polling-Abfrage fehl (Token abgelaufen und Refresh fehlgeschlagen, Microsoft-API nicht erreichbar, Ordner nicht mehr vorhanden), wird das in `EmailWatchConfig.letzter_fehler` vermerkt und in den Settings sichtbar angezeigt – die restliche App bleibt davon unberührt (analog zur bestehenden Vorgabe, dass ein Claude-API-Ausfall die App nicht lahmlegt, siehe Updates-Abschnitt).
    - **Datenschutz-Einordnung:** siehe Datenschutz-Architektur-Bullet in den Rahmenbedingungen oben – neue externe Abhängigkeit (Microsoft Graph API) zusätzlich zur Claude API.
    - **Hintergrund-Ausführung:** Das Polling nutzt einen einfachen periodischen Scheduler (z. B. `APScheduler` innerhalb des FastAPI-Prozesses), keine externe Job-Queue – konsistent mit der bestehenden Vorgabe, Redis/Celery/RQ im MVP zu vermeiden (siehe "Bewusst NICHT für den MVP einführen"). Der Scheduler-Task ist von der bestehenden Background-Task-Recovery-Logik (siehe Kernfunktion 4) konzeptionell getrennt, da er periodisch statt einmalig pro Dokument läuft.
...
- Kein automatisches Einlesen aus Teams/Jira (Outlook-Ordnerüberwachung ist seit der entsprechenden Erweiterung Teil des MVP, siehe Kernfunktion 12 – die Ausnahme gilt nur noch für Outlook)
